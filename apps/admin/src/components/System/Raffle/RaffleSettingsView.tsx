...
export const RaffleSettingsView = forwardRef<
  RaffleSettingsViewRef,
  RaffleSettingsViewProps
  ...
  useImperativeHandle(ref, () => ({
    handleSave: async () => {
      ...
      try {
        ...
        showToast(
          `Módulo de rifas ${isEnabled ? "habilitado" : "deshabilitado"} correctamente`,
          "success",
        );
  ...
  return (
    ...
    >
      ...
      <div className="bg-brand-50 border border-brand-100 p-8 rounded-[2.5rem] flex gap-5 items-start shadow-sm dark:shadow-none">
        ...
        <div>
          <h4 className="text-lg font-black text-brand-900 tracking-tight">
            Configuración Global de Rifas
          </h4>
          <p className="text-sm text-brand-800/80 mt-1.5 leading-relaxed font-medium">
            Este interruptor controla la visibilidad de la sección de rifas en
            toda la plataforma. Al desactivarlo, se ocultarán los accesos tanto
            en este panel administrativo como en la tienda pública para los
            clientes.
          </p>
        </div>
      </div>
      ...
      <div className="bg-bg-card border border-border-main rounded-[2.5rem] p-8 shadow-sm dark:shadow-none">
        <div className="flex items-center justify-between mb-6 pb-6 border-b border-border-main gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-text-main uppercase tracking-widest text-sm leading-tight">
                Estado del Módulo
              </h3>
              <p className="text-[10px] text-stone-400 font-bold uppercase mt-1">
                Activa o desactiva las rifas en el sistema
              </p>
            </div>
          </div>
          ...
          >
            <div
              className={`absolute top-1 w-5 h-5 rounded-full bg-bg-card shadow-sm dark:shadow-none transition-all ${isEnabled ? "left-8" : "left-1"}`}
            />
          </button>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3 px-4 py-3 bg-bg-muted rounded-2xl border border-border-main">
            <ShieldCheck
              size={18}
              className={isEnabled ? "text-emerald-500" : "text-stone-300"}
            />
      ...
      <div className="bg-amber-50 border border-amber-100 p-8 rounded-[2.5rem] flex gap-5 items-start shadow-sm dark:shadow-none">
        ...
        <div>
          ...
          <p className="text-sm text-amber-800/80 mt-1.5 leading-relaxed font-medium">
            Solo los usuarios con privilegio de <strong>Superadmin</strong>{" "}
            pueden modificar este ajuste. Asegúrate de que no haya rifas activas
            antes de deshabilitar el módulo.
          </p>
        </div>
      </div>
    </div>
