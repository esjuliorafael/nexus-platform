...
export const ProductForm: React.FC<ProductFormProps> = ({ initialData, onCancel, onSave, onValidationChange, showToast }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const payload: any = {
        type: productType.toUpperCase() === 'AVE' ? 'BIRD' : 'ARTICLE',
        name,
        price: parseFloat(price),
        description,
        saleStatus: status.toUpperCase(),
        thumbnail: coverUrl,
        // For simplicity, we'll map current galleryUrls
        // In a real scenario we'd handle new uploads via a separate media API
      };
