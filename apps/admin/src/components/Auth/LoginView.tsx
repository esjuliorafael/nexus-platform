...
interface LoginViewProps {
  onLoginSuccess: (userData: any, jwtToken: string) => void;
...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, showToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const responseData = await apiAuth.login({ username: username.trim(), password });
      ...
      showToast(err.response?.data?.message || err.message || 'No se pudo conectar con el servidor.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
