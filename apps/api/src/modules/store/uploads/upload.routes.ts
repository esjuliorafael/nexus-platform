...
export async function uploadRoutes(server: FastifyInstance) {
  ...
  server.post("/", async (request, reply) => {
    ...
    if (!data)
      return reply.status(400).send({ message: "Selecciona un archivo." });
    ...
    try {
      ...
      if (data.file.truncated) {
        return reply
          .status(413)
          .send({ message: "El archivo supera el limite de 500 MB." });
      }
      ...
      const statusCode =
        error?.statusCode ||
        (error?.code === "FST_REQ_FILE_TOO_LARGE" ? 413 : 500);
  ...
  server.post("/:id/complete", async (request, reply) => {
    try {
      ...
      if (error?.issues) {
        return reply
          .status(400)
          .send({ message: "Identificador de asset invalido." });
      }
      const statusCode = error?.statusCode || 500;
      return reply.status(statusCode).send({
  ...
  server.get("/:id", async (request, reply) => {
    try {
      ...
      if (!asset)
        return reply.status(404).send({ message: "Asset no encontrado." });
      return asset;
    } catch (error: any) {
      if (error?.issues) {
        return reply
  ...
  server.post("/:id/reprocess", async (request, reply) => {
    try {
      const { id } = assetParamsSchema.parse(request.params);
      return await mediaAssetService.reprocessVideo(id);
    } catch (error: any) {
      if (error?.issues) {
        return reply
          .status(400)
      ...
      return reply.status(statusCode).send({
        message:
          error?.message || "No se pudo reintentar la optimizacion del video.",
      });
    }
  });
  ...
  server.delete("/:id", async (request, reply) => {
    try {
      ...
      if (!deleted) {
        return reply
          .status(409)
          .send({ message: "El asset sigue asociado a contenido." });
      }
      return { success: true };
    } catch (error: any) {
      if (error?.issues) {
