...
interface CartState {
  ...
  clearCart: () => void;
  replaceCart: (items: CartItem[], coupon?: CartCoupon | null) => void;
...
export const useCartStore = create<CartState>()(
  persist(
    (set, get) => ({
      ...
      addItem: (item) => {
        ...
        if (existing) {
          ...
          set({
            items: [
              ...get().items,
              { ...item, quantity: item.type === "bird" ? 1 : item.quantity },
            ],
            coupon: null,
          });
          return "added";
      ...
      replaceCart: (items, coupon = null) => set({ items, coupon }),
      ...
      getDiscountTotal: () =>
        Math.min(get().coupon?.discountTotal || 0, get().getTotalPrice()),
      getCartTotalAfterDiscount: () =>
        Math.max(0, get().getTotalPrice() - get().getDiscountTotal()),
    }),
    {
      name: "nexus_cart",
    },
