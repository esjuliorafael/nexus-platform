...
export function ProductPage() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><Spinner className="w-12 h-12" /></div>;
  if (!product) return <div className="h-screen flex items-center justify-center font-bold">Producto no encontrado</div>;

  const isAvailable = product.saleStatus === 'AVAILABLE';

  return (
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-12">
      <Link to="/store" className="inline-flex items-center gap-2 text-stone-500 hover:text-brand-500 font-bold transition-colors">
          ...
          {product.type === 'BIRD' && (
            <div className="grid grid-cols-2 gap-4 bg-stone-50 p-6 rounded-[2rem] border border-stone-100">
              ...
              <div className="space-y-1">
                ...
                <p className="font-bold text-stone-700">{product.age || 'N/A'}</p>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] font-black uppercase text-stone-400 tracking-widest">Propósito</span>
                <p className="font-bold text-stone-700">{product.purpose || 'N/A'}</p>
              </div>
            </div>
          )}
